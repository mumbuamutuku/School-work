...
def get_timezone():
    """function that uses the babel.timezoneselector decorator.
    """
    try:
        timezon = request.args.get("timezone")
        if timezon:
            return pytz.timezone(timezon)
        user = request.args.get("login_as")
        if user:
            timezon = users.get(int(user)).get('timezone')
            if timezon:
                return pytz.timezone(timezon)
        timezon = request.headers.get("timezone")
        if timezon:
            return pytz.timezone(timezon)
